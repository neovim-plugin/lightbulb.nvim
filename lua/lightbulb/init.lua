---
--- *lightbulb.nvim*: VSCode 💡 for neovim's built-in LSP.

--- Fork of https://github.com/kosayoda/nvim-lightbulb with the lates API
---
--- --- Quickstart ---
---
...
---   })
---
--- See |lightbulb-config| for available config settings.
---
---
--- --- Modify Highlights ---
---
--- To modify highlights, configure the corresponding highlight group.
--- See |lightbulb-config| for a list of highlights used.
---
--- Example:
--- >
...
local lightbulb_config = require("lightbulb.config")

local get_clients = tonumber(vim.inspect(vim.version().minor)) >= 10 and vim.lsp.get_clients
...
--- Module setup.
---
--- Optional. Any configuration can also be passed to |Lightbulb.update_lightbulb|.
---
---@param config table|nil Partial or full configuration table. See |lightbulb-config|.
---
---@usage `require('lightbulb').setup({})`
Lightbulb.setup = function(config)
	_G.Lightbulb = Lightbulb
	lightbulb_config.set_defaults(config)
end

--- Update the lightbulb float.
---
---@param opts table Partial or full configuration table. See |lightbulb-config|.
...
--- Any configuration provided overrides the defaults passed to |Lightbulb.setup|.
---
---@param bufnr number|nil Buffer handle. Defaults to current buffer.
---
---@usage `require('lightbulb').get_status_text()`
Lightbulb.get_status_text = function(bufnr)
	bufnr = bufnr or vim.api.nvim_get_current_buf()
	return vim.F.npcall(vim.api.nvim_buf_get_var, bufnr, "current_lightbulb_status_text") or ""
end

Lightbulb.clear_lightbulb = function(bufnr)
	local extmark_id = vim.b[bufnr].lightbulb_extmark
	if extmark_id ~= nil then
		vim.api.nvim_buf_del_extmark(bufnr, LIGHTBULB_NS, extmark_id)
	end
...
---@param config table|nil Partial or full configuration table. See |lightbulb-config|.
---
---@usage `require('lightbulb').update_lightbulb({})`
Lightbulb.update_lightbulb = function(config)
...
---
--- Display debug information related to lightbulb.
...
---@param config table|nil Partial or full configuration table. See |lightbulb-config|.
---
---@usage `require('lightbulb').debug({})`
Lightbulb.debug = function(config)
	local opts = lightbulb_config.build(config, false)
	opts.ignore_id = {}
