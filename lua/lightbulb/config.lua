...
---
---@param opts table|nil Partial or full configuration table. See |lightbulb-config|.
---
---@private
M.set_defaults = function(opts)
  ...
  if default_config.autocmd.enabled then
    ...
    vim.api.nvim_create_autocmd(default_config.autocmd.events, {
      ...
      group = id,
      desc = "lua require('lightbulb').update_lightbulb()",
      callback = require("lightbulb").update_lightbulb,
    })
  end
  ...
  if default_config.hide_in_unfocused_buffer then
    vim.api.nvim_create_autocmd({ "WinLeave" }, {
      pattern = { "*" },
      group = id,
      desc = "lua require('lightbulb').clear_lightbulb()",
      callback = function(args)
        require("lightbulb").clear_lightbulb(args.buf)
      end,
    })
  end
...
---
---@param opts table Configuration table. See |lightbulb-config|.
